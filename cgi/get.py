...
script_name = os.environ.get('SCRIPT_NAME',    '/cgi/get_test.py')
...
def result_row(key, val):
    return (f'<div class="result-row">'
            f'<div class="result-key">{esc(key)}</div>'
            f'<div class="result-val">{esc(val)}</div>'
            f'</div>')

def env_line(key, val):
    return (f'<div class="env-line">'
...
if method == 'GET':
    ...
    fields_html  = '<span class="section-label">Query Parameters</span>'
    fields_html += '<div class="result-block">'
    if flat:
        for k, v in flat.items():
            fields_html += result_row(k, v)
    else:
        fields_html += '<div class="result-row"><div class="result-val" style="opacity:.4">No query parameters</div></div>'
    fields_html += '</div>'

    env_html  = '<span class="section-label">Request Info</span>'
    env_html += '<div class="env-block">'
    env_html += env_line('METHOD',       method)
    env_html += env_line('QUERY_STRING', query_str or '(empty)')
    env_html += env_line('REMOTE_ADDR',  remote_addr)
    env_html += env_line('SERVER',       f'{server_name}:{server_port}')
    env_html += env_line('USER_AGENT',   user_agent)
    env_html += env_line('PYTHON',       sys.version.split()[0])
    env_html += '</div>'
    ...
    status_dot   = 's-warn'
    status_label = 'Unexpected Method'
    title        = 'Awaiting GET'

    body_html  = '<span class="section-label">Instructions</span>'
    body_html += '<div class="result-block">'
    body_html += result_row('expected', 'GET')
    body_html += result_row('example',  f'{script_name}?name=john&age=30')
    ...
    body_html += env_line('SERVER',      f'{server_name}:{server_port}')
    body_html += env_line('PYTHON',      sys.version.split()[0])
    body_html += '</div>'
...
<title>CGI — {esc(script_name.split('/')[-1])}</title>
  ...
  <main>
    <div class="card">
      <div class="card-header">
        <div>
          <div class="tag">Python · CGI Response</div>
